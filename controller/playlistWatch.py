...
class playlistWatch:
    def __init__(self, playlist_list:list, tweet_API, youtube_API):
        self.Play_Lists = playlist_list
        self.Twitter_API = tweet_API
        self.youtubeObject = youtube_API
    ...
    # ##twitterテストアカウント
    # self.CONSUMER_KEY = "OgUS1y3y7vuxy54NoKZvlOdq9"
    # self.CONSUMER_SECRET = "hCRRA4WX5cEe50ScugCkF4MvFJeFvU8YFAiwGDBi2vkJ9PqyZL"
    # self.ACCESS_TOKEN = "1000217159446945793-0LiJPmZvvyfaQvNhiY1pgL52pCTnuW"
    # self.ACCESS_TOKEN_SECRET = "enagarkdimg1cdR4w8ZFZhEr0kyjVj8ekNRzmiZviz4z8"
    ...
    def tweet(self, message):
        ...
        TWEET_TEXT = message
        try :
            tweet_status = self.Twitter_API.update_status(TWEET_TEXT)
    ...
    def tweetWithIMG(self, message,img_url):
        ...
        try :
            ...
            tweet_status = self.Twitter_API.update_with_media(filename=FILE_NAME, status=TWEET_TEXT)
            if tweet_status == 200: #成功
                pprint("Succeed!")
                result = True
            else:
    ...
    def main(self):
        ...
        # DB管理バージョンーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー
            ...
            for entry in feedparser.parse(url).entries:
                ...
                imgPro = ImageProcessing.ImageProcessing(entry['media_thumbnail'][0]['url'],TMP_DIR=self.LIVE_TMB_TMP_DIR, IMG_DIR=self.LIVE_TMB_IMG_DIR)
                if not result:
                # 同じIDがない(新規)
                    newdata = True
                ...
                # 同じIDがある(既存)
                    ...
                    if time_lag.total_seconds() >= self._TIMELAG:
                        ...
                        if not imgPro.imageComparison_hash_other():
                            # ---------------------------------
                            shutil.move(self.OTHER_TMB_TMP_DIR, imgPro._TMB_IMG_FilePath)
                            update = True
                            updateKind += 'image'
